...
class App extends Component{
    ...
    setInput = (title, input) => {
        let clone = this.state.inputs.slice(0);
    ...
    renderScrollSink = (currentStyles) => {
        return (
            <ScrollSink changeScrollTop={this.changeScrollTop} scrollTop={currentStyles.scrollTop}/>
        )
    }
    ...
    render(){
        let contactData = this.state.inputs.map((item, i) => {
            return (<Input key={i} setInput={this.setInput} setScroll={this.setScroll} showCopied={this.state.showCopied} {...item}/>);
        });
